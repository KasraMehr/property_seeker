...
export default function CallDetailModal({ isOpen, onClose, call }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="lg" title="جزئیات تماس" className="h-[70vh]">
      {/* Header */}
      <div className="flex shrink-0 items-center gap-3 mb-4 pb-4 border-b border-border">
        ...
        <div className="min-w-0 flex-1">
          <h3 className="text-base font-bold text-foreground">تماس با {call.customer?.full_name || "—"}</h3>
          <div className="flex items-center gap-2 mt-1">
            <StatusBadge status={call.call_type} config={CALL_TYPE_CONFIG} size="sm" variant="soft" />
            <StatusBadge status={call.result} config={CALL_RESULT_CONFIG} size="sm" variant="soft" />
          </div>
        </div>
      </div>
