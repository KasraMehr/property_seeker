...
export const SCRAPER_RUN_TABLE_COLUMNS = [
  {
    ...
    width: "w-36",
    cell: ({ id }) => (
      <span className="text-xs font-mono text-muted-foreground truncate max-w-30" title={id}>
        {id?.slice(0, 8)}...
      </span>
    ),
  },
  ...
  },
  {
    ...
    width: "w-24",
    filterKey: "status",
    cell: ({ status }) => <StatusBadge status={status} config={INGESTION_RUN_STATUS_CONFIG} />,
  },
  {
    key: "discovered",
    header: "کشف / پردازش",
    ...
    cell: ({ discovered_count, processed_count, new_count, changed_count }) => (
      <div className="flex flex-col text-xs">
        <span className="text-muted-foreground">{processed_count || 0} / {discovered_count || 0} پردازش</span>
  ...
  },
  {
    ...
    cell: ({ finished_at, status }) => {
      if (!finished_at) return <span className="text-xs text-amber-600">در حال اجرا...</span>;
