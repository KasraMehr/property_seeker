...
class AdvertiserClassificationTests(TestCase):
    ...
    def test_accepts_only_the_two_category_labels(self):
        ...
        self.assertIn(
            "untrusted data",
            owner_client.messages[0]["content"],
        )
    ...
    def test_timeout_rate_limit_and_permanent_http_errors_are_typed(self):
        ...
        rate_response = httpx2.Response(
            429,
            request=request,
        )
        denied_response = httpx2.Response(
            401,
            request=request,
        )
    ...
    def test_success_persists_model_hash_and_timestamp(self):
        ...
        self.assertEqual(
            self.listing.advertiser_classification_model,
            "gpt-5.6-luna",
        )
        ...
        self.assertIsNotNone(
            self.listing.advertiser_classified_at
        )

    def test_exhausted_transient_failure_is_non_blocking_and_recorded(self):
        with patch(
            "ingestion.services.advertiser_classification.classify_description",
            side_effect=AdvertiserClassificationTransientError(
                "temporary"
            ),
        ) as classify_mock:
            outcome = classify_listing_synchronously(
    ...
    def test_permanent_failure_does_not_retry(self):
        with patch(
            "ingestion.services.advertiser_classification.classify_description",
            side_effect=AdvertiserClassificationPermanentError(
                "denied"
            ),
        ) as classify_mock:
            classify_listing_synchronously(
    ...
    def test_empty_description_stays_pending_without_calling_gapgpt(
        self,
    ):
        self.listing.description = ""
        self.listing.save(
...
class AdvertiserClassificationIntegrationTests(TestCase):
    ...
    def payload(
        self,
        description="مالک هستم، بدون واسطه",
    ):
    ...
    def test_unchanged_description_preserves_classification_and_change_invalidates_it(
        self,
    ):
        ...
        listing.advertiser_description_hash = description_hash(
            listing.description
        )
        ...
        self.assertEqual(
            changed.advertiser_description_hash,
            "",
        )

    def test_classification_queue_failure_does_not_escape_into_ingestion(
        self,
    ):
    ...
    def test_serializers_and_server_filter_expose_the_category(
        self,
    ):
        ...
        list_data = ListingListSerializer(
            owner
        ).data

        detail_data = ListingDetailSerializer(
            owner
        ).data

        self.assertEqual(
            list_data["advertiser_type"],
            "owner",
        )
        ...
        # ListingSelector.for_user() expects a real User
        # and specifically accesses user.is_owner.
        User = get_user_model()
        ...
        # Set the authenticated user on the DRF Request itself.
        # Setting request.user before wrapping it is not enough,
        # because DRF can authenticate the request and replace it
        # with AnonymousUser.
    ...
    def test_backfill_is_resumable_and_honors_limit(
        self,
        delay_mock,
    ):
        up_to_date = Listing.objects.create(
            ...
            advertiser_description_hash=description_hash(
                "مالک"
            ),
        )
    ...
    def test_force_sync_backfill_does_not_require_celery(
        self,
        sync_mock,
    ):
        listing = Listing.objects.create(
    ...
    def test_csv_export_contains_category_and_processing_status(
        self,
    ):
        listing = Listing.objects.create(
            source=self.source,
