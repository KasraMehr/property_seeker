...
class ListingAdmin(admin.ModelAdmin):
    ...
    def promote_view(self, request, object_id):
        ...
        form = PromoteListingForm(
            request.POST or None, listing=listing, actor=request.user
        )
        if request.method == "POST" and form.is_valid():
            ...
            messages.success(
                request, f"Created property {property_record.property_code}."
            )
            return redirect(
                reverse("admin:properties_property_change", args=[property_record.pk])
            )
        return render(
            request,
